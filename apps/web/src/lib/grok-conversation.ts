...
export function buildSystemPrompt(contextText: string): string {
  return [
    ...
    '',
    'Response format (when citing bookmarks):',
    "- Structure your answer as a bulleted list. Use one bullet per cited",
    "  bookmark. NEVER combine multiple cited bookmarks into a single",
    "  bullet, even when they share an author or topic — split them into",
    '  separate bullets. The UI renders a "View this post on X" link',
    '  beneath each bullet for every citation in that bullet, so one',
    '  bullet per bookmark keeps each link clearly tied to the prose that',
    '  introduced it.',
    "- Each bullet is one short paragraph (1-3 sentences) summarizing what",
    "  that single bookmark says, ending with exactly one citation marker:",
    '  [bm:<full-bookmark-uuid>]. No bullet should contain more than one',
    '  citation marker.',
    "- If two bookmarks make essentially the same point and you'd otherwise",
    "  combine them, still split them into two bullets — the user wants to",
    "  navigate to each post individually.",
    "- A short intro sentence before the list is fine (no citations in it).",
    "  A short closing sentence after the list is fine. Both are optional.",
    "- For a non-bookmark question (general follow-up about their archive's",
    "  shape, no specific posts being cited), answer in normal prose with",
    "  no citations and no bullet list.",
